...
class qr_links_proc:
    ...
    def complete_links_save(self, request, session, root_path):
        ...
        if welcome_tmp_key and welcome_tmp_name:
            ...
            _move_specs.append((
                f"links/_tmp/{welcome_tmp_key}/{welcome_tmp_name}",
                f"links/{new_id}/welcome{ext}",
                {"fk_user_id": fk_user_id, "qrcard_id": new_id, "qr_type": "links", "file_name": f"welcome{ext}"},
                "welcome"
            ))
        else:
            ...
            if _direct_welcome and _direct_welcome.filename:
                ...
                if _dw_size <= 1 * 1024 * 1024:
                    ...
                    _upload_specs.append((_direct_welcome, f"links/{new_id}/{_dw_name}", {"fk_user_id": fk_user_id, "qrcard_id": new_id, "qr_type": "links", "file_name": _dw_name}, "welcome"))
            else:
                    ...
                    if os.path.isfile(local_path):
                        try:
                            ...
                            with open(local_path, "rb") as f:
                                _upload_specs.append((io.BytesIO(f.read()), f"links/{new_id}/{unique_welcome_name}", {"fk_user_id": fk_user_id, "qrcard_id": new_id, "qr_type": "links", "file_name": unique_welcome_name}, "welcome"))
                        except Exception:
                            pass
        ...
        if cover_tmp_key and cover_tmp_name:
            ...
            _move_specs.append((
                f"links/_tmp/{cover_tmp_key}/{cover_tmp_name}",
                f"links/{new_id}/{unique_cover_name}",
                {"fk_user_id": fk_user_id, "qrcard_id": new_id, "qr_type": "links", "file_name": unique_cover_name},
                "cover"
            ))
        else:
            ...
            if _direct_cover and _direct_cover.filename:
                ...
                if _dc_size <= 2 * 1024 * 1024:
                    ...
                    _upload_specs.append((_direct_cover, f"links/{new_id}/{_dc_name}", {"fk_user_id": fk_user_id, "qrcard_id": new_id, "qr_type": "links", "file_name": _dc_name}, "cover"))
            else:
                ...
                if ac_url and (ac_url.startswith("http://") or ac_url.startswith("https://")):
                    ...
                    if os.path.isfile(local_path):
                        try:
                            unique_cover_name = f"links_cover_img_{uuid.uuid4().hex[:12]}{ext}"
                            with open(local_path, "rb") as f:
                                _upload_specs.append((io.BytesIO(f.read()), f"links/{new_id}/{unique_cover_name}", {"fk_user_id": fk_user_id, "qrcard_id": new_id, "qr_type": "links", "file_name": unique_cover_name}, "cover"))
                        except Exception:
                            pass
        ...
        for tag, url in _db_updates.items():
            if tag == "welcome":
                try:
                    ...
                    self.mgdDB.db_qrcard_links.update_one({"qrcard_id": new_id}, {"$set": {"welcome_img_url": url}}, upsert=True)
                except Exception:
                    pass
            elif tag == "cover":
                try:
                    self.mgdDB.db_qrcard.update_one({"qrcard_id": new_id}, {"$set": {"Links_cover_img_url": url}})
                    self.mgdDB.db_qrcard_links.update_one({"qrcard_id": new_id}, {"$set": {"Links_cover_img_url": url}}, upsert=True)
                except Exception:
                    pass

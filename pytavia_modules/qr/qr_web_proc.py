...
class qr_web_proc:
    ...
    def add_qrcard_static(self, params):
        """Create a new web-static QR card (URL encoded directly, no redirect)."""
        try:
            fk_user_id = params.get("fk_user_id")
            name = params.get("name", "Untitled QR")
            url_content = params.get("url_content", "")
            ...
            qrcard_rec["qrcard_id"] = qrcard_id
            qrcard_rec["fk_user_id"] = fk_user_id
            qrcard_rec["qr_type"] = "web-static"
            qrcard_rec["name"] = name
            qrcard_rec["url_content"] = url_content
            qrcard_rec["short_code"] = ""
            qrcard_rec["design_data"] = {}
            qrcard_rec["qr_image_url"] = ""
            qrcard_rec["stats"] = {"scan_count": 0}
            qrcard_rec["scan_limit_enabled"] = False
            ...
            err_trace = traceback.format_exc()
            self.webapp.logger.debug(err_trace)
            return {"message_action": "ADD_QRCARD_FAILED", "message_desc": "An internal error occurred.", "message_data": {"trace": err_trace}}

    def add_qrcard_text(self, params):
        """Create a new text QR card (plain text encoded directly)."""
        try:
            fk_user_id = params.get("fk_user_id")
            name = params.get("name", "Untitled QR")
            ...
            qrcard_rec["qrcard_id"] = qrcard_id
            qrcard_rec["fk_user_id"] = fk_user_id
            qrcard_rec["qr_type"] = "text"
            qrcard_rec["name"] = name
            qrcard_rec["text_content"] = text_content
            qrcard_rec["url_content"] = ""
            qrcard_rec["short_code"] = ""
            qrcard_rec["design_data"] = {}
            qrcard_rec["qr_image_url"] = ""
            qrcard_rec["stats"] = {"scan_count": 0}
    ...
    def edit_qrcard_static(self, params):
        """Update a web-static QR card."""
        try:
            fk_user_id = params.get("fk_user_id")
            qrcard_id = params.get("qrcard_id")
            self.mgdDB.db_qrcard.update_one(
                {"fk_user_id": fk_user_id, "qrcard_id": qrcard_id},
                {"$set": {"name": params.get("name"), "url_content": params.get("url_content", "")}},
            )
            self.mgdDB.db_qr_index.update_one(
                {"fk_user_id": fk_user_id, "qrcard_id": qrcard_id},
    ...
    def edit_qrcard_text(self, params):
        """Update a text QR card."""
        try:
            fk_user_id = params.get("fk_user_id")
            qrcard_id = params.get("qrcard_id")
            self.mgdDB.db_qrcard.update_one(
                {"fk_user_id": fk_user_id, "qrcard_id": qrcard_id},
                {"$set": {"name": params.get("name"), "text_content": params.get("text_content", "")}},
            )
            self.mgdDB.db_qr_index.update_one(
                {"fk_user_id": fk_user_id, "qrcard_id": qrcard_id},
                {"$set": {"name": params.get("name")}},

    ...
    def complete_pdf_save(self, request, session, root_path):
        ...
        if cover_tmp_key:
            ...
            if _direct_cover and _direct_cover.filename:
                ...
                _direct_cover_size = _direct_cover.tell()
                _direct_cover.seek(0)
                if _direct_cover_size <= 2 * 1024 * 1024:
                    _dc_ext = os.path.splitext(_direct_cover.filename)[1].lower() or ".jpg"
                    if _dc_ext not in (".jpg", ".jpeg", ".png", ".gif", ".webp"):
                        _dc_ext = ".jpg"
                    _dc_name = f"pdf_cover_img_{uuid.uuid4().hex[:12]}{_dc_ext}"
                    try:
                        cover_url = r2.upload_file(_direct_cover, f"pdf/{new_qrcard_id}/{_dc_name}", track_meta={"fk_user_id": fk_user_id, "qrcard_id": new_qrcard_id, "qr_type": "pdf", "file_name": _dc_name})
                        self.mgdDB.db_qrcard.update_one(
                            {"qrcard_id": new_qrcard_id},
                            {"$set": {"pdf_t1_header_img_url": cover_url, "pdf_t3_circle_img_url": cover_url, "pdf_t4_circle_img_url": cover_url}},
                        )
